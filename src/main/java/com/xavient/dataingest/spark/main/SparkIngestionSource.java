/* This class contains the main method, it is to be executed in the internal cluster */
...
public class SparkIngestionSource implements Serializable {
	...
	public static void main(String[] args) throws ParserConfigurationException, IOException, DataIngestException {
		...
		try {
			...
			JavaPairReceiverInputDStream<String, String> stream = KafkaUtils.createStream(jssc,
					appArgs.getProperty(Constants.SOURCE_ZK_HOST) + ":" + appArgs.getProperty(Constants.SOURCE_ZK_PORT),
	...
	private static Map<String, Integer> getKafkaTopics(AppArgs appArgs) {
		...
		topics.put(appArgs.getProperty(Constants.SOURCE_KAFKA_TOPIC), 1);
		return topics;
	}
